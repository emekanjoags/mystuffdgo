...
from autoUpdate.update_api import *
...
def mybets_view(request):
    ...
    if num_bets >= 5:
        try:
            users_get_bonus = RafflePlayer.objects.filter(user=request.user, raffle__ended=False, get_bonus=0)
            # user_get_bonus = users_get_bonus[0]pog
            if users_get_bonus:
                for i in users_get_bonus:
                    i.get_bonus = 1
                    i.save()            
...
class StartFirstGame(APIView):
    def post(self, request):
        match_check = request.data.get('match_check')
        start_time = request.data.get('start_time')
        print(start_time)
        if match_check == 'yes':
            #UpdateApi().start_today_game
            return Response(data='everything went well', status=status.HTTP_200_OK)
        else:
            return Response(data="erro with match_check", status=status.HTTP_200_OK)