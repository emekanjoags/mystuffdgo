        ...
        if wallet.balance >= koboless_amount:
            ...
            if balance > amount:
                ...
                if create_recipient[1]:
                    print("create_receipirnt: ", create_recipient)
                    print('create reciepirn [1]: ', create_recipient[1])
                    ...
                    print('rres: ', response)
                    print('receipirn: ', create_recipient[3]['recipient_code'])
                    ...
                    if response[1]:
                        ...
                        if response[3]['status'] == 'success':
                            print('rsponse 3 suc: ', response[3])
                            withdrawal.transaction_status = 1
                            withdrawal.is_confirmed = True

                            wallet.balance = F('balance') - koboless_amount
                            wallet.save()
                            print('suc debited: ', wallet.balance)
                        elif response[3]['status'] == 'otp':
                            wallet.balance = F('balance') - koboless_amount
                            wallet.save()
                            print('otp debited: ', wallet.balance)

                        elif response[3]['status'] == 'pending':
                            print('response pen 3: ', response[3])
                            withdrawal.transaction_status = 0
                            wallet.balance = F('balance') - koboless_amount
                            wallet.save()
                            print('pen debited: ', wallet.balance)
                        elif response[3]['status'] == 'failed':
